...
const createSchema = z.object({ body: z.object({
  ...
  couponCode: z.string().trim().max(30).optional(), paymentMethod: z.enum(['COD', 'ONLINE']).default('COD'),
...
router.post('/:id/cancel', async (req, res, next) => {
  try {
    ...
    if (order.paymentMethod === 'ONLINE' && order.paymentStatus === 'PAID') throw new AppError(409, 'REFUND_REQUIRED', 'Paid online orders must be refunded before cancellation');
