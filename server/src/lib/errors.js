...
export const errorHandler = (err, req, res, _next) => {
  ...
  if (['P2021', 'P2022'].includes(err?.code) || (err?.name === 'PrismaClientValidationError' && /manual(Submissions|Destination)|manualPayment(Channel|Submission)/i.test(err.message || ''))) {
    req.log?.error({ err }, 'database schema/client is out of date');
    return res.status(503).json({ error: { code: 'DATABASE_SCHEMA_OUT_OF_DATE', message: 'Payment database schema is out of date. Stop the server, run npm run db:setup, then start npm run dev again.' } });
  }
