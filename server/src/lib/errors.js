...
export const errorHandler = (err, req, res, _next) => {
  ...
  if (err?.code === 'P2034') return res.status(409).json({ error: { code: 'ORDER_CONFLICT', message: 'Availability changed while placing the order. Please review the latest slot and try again.' } });
