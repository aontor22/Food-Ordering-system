...
export const errorHandler = (err, req, res, _next) => {
  ...
  if (['P2021', 'P2022'].includes(err?.code) || (err?.name === 'PrismaClientValidationError' && /manual(Submissions|Destination)|manualPayment(Channel|Submission)|imagePublicId|pointsBalance|Review|Loyalty/i.test(err.message || ''))) {
