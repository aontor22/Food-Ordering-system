...
export default function AdminCustomers() {
  ...
  return <>
    ...
    <section className="admin-card">
      {visible.length ? <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Account</th><th>Role</th><th>Joined</th><th>Orders</th><th>Lifetime value</th><th>Status</th><th className="align-right">Actions</th></tr></thead>
        <tbody>{visible.map(user => <tr key={user.id}>
          <td><strong>{user.name}</strong><small>{user.email}</small></td><td>{user.role === 'ADMIN' ? 'Administrator' : 'Customer'}</td><td>{formatDate(user.createdAt)}</td><td><strong>{user.orderCount}</strong></td><td><strong>{formatCurrency(user.lifetimeValueCents / 100)}</strong></td><td><StatusBadge value={user.isActive ? 'ACTIVE' : 'DISABLED'} /></td>
