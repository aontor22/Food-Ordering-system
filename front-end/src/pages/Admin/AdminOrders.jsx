...
function OrderRows({ order, expanded, onExpand, onStatus, busy }) {
  ...
  return <>
    ...
    {expanded && <tr className="order-expanded"><td colSpan="6"><div className="order-detail">
      ...
      <div><h4>Delivery address</h4>{order.deliveryZoneName && <p><strong>Zone:</strong> {order.deliveryZoneName}<br /><strong>Delivery fee:</strong> {formatCurrency(order.deliveryFeeCents / 100, order.payment?.currency)}</p>}<p>{order.firstName} {order.lastName}<br />{order.street}<br />{order.city}, {order.state} {order.postalCode}<br />{order.country}<br />{order.phone}</p>{order.notes && <p><strong>Note:</strong> {order.notes}</p>}</div>
