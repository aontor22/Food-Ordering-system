  ...
  return <div className="orders-page">
    ...
    {loading ? <div className="orders-loading"><span />Loading your orders…</div> : <div className="orders-list">
      {orders.map(order => <article className="order-card surface-card" key={order.id}>
        ...
        <div className="order-items-preview">{order.items.map(item => <div key={item.id}><span>{item.quantity}×</span><p>{item.productName}</p><strong>{formatCurrency(item.lineTotalCents / 100, order.payment?.currency)}</strong></div>)}</div>
        ...
        {(order.paymentMethod === 'MANUAL' || (order.paymentMethod === 'ONLINE' && !['PAID', 'REFUNDED', 'REVIEW'].includes(order.paymentStatus) && !['CANCELLED', 'DELIVERED'].includes(order.status)) || (['PENDING', 'CONFIRMED'].includes(order.status) && !(order.paymentMethod !== 'COD' && (order.paymentStatus === 'PAID' || order.paymentStatus === 'REVIEW' || (order.payment?.provider === 'SSLCOMMERZ' && order.paymentStatus === 'PROCESSING'))))) && <div className="order-customer-actions">
          ...
          {['PENDING', 'CONFIRMED'].includes(order.status) && !(order.paymentMethod !== 'COD' && (order.paymentStatus === 'PAID' || order.paymentStatus === 'REVIEW' || (order.payment?.provider === 'SSLCOMMERZ' && order.paymentStatus === 'PROCESSING'))) && <button className="button button-secondary" disabled={Boolean(busy)} onClick={() => cancel(order)}>{busy === `cancel:${order.id}` ? 'Cancelling…' : 'Cancel order'}</button>}
        </div>}
      </article>)}
    </div>}
  </div>;
